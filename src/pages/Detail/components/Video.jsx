...
const Video = ({ currentTime, setCurrentTime, player, setPlayer }) => {
  const videoId = "NDsO1LT_0lw";
  const { englishCaption, koreanCaption, isLoading, isError, error } =
    useCaptionDataQuery(videoId);
  const { data, isLoading: videoLoading } = useVideoDetailsQuery(videoId);
  ...
  const test = () => {
    console.log(englishCaption);
    console.log(koreanCaption);
    console.log(data);
  };
  const opts = {
    height: "100%",
      ...
      {!videoLoading && (
        <div className="mt-4">
          <h1 className="text-xl font-bold">
            {data[0].title}{" "}
            <button className="border border-black-400" onClick={() => test()}>
              {" "}
              caption test
            </button>
          </h1>
